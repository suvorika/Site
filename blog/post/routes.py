...
def post(slug):
    ...
    if request.method == "POST" and form_comment.validate_on_submit():
        ...
        comment = Comment(
            username=username, body=form_comment.body.data, post_id=post.id
        )
...
def search():
    try:
        ...
        return render_template(
            "post/search.html", search_posts=search_posts, title="Поиск"
        )
...
def update_post(slug):
    ...
    return render_template(
        ...
        title="Обновление " + post.title,
        form_post_update=form,
...
def update_comment(comment_id):
    ...
    if current_user.is_admin or comment.username == current_user.username:
        ...
        abort(403)
    return render_template(
        "post/update_comment.html",
        form_comment_update=form,
...
def category(category_str):
    ...
    return render_template(
        "post/all_posts_category.html",
        post_category=posts_category,
        current_category=current_category,
        title="Рубрика " + category_str,
    )
...
def tag(tag_str):
    ...
    return render_template(
        "post/all_post_tag.html",
        name_tags=name_tags,
        current_tag=current_tag,
        title="Статьи тега " + current_tag.name,
    )
